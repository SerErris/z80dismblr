...
engineering with `z80map` more convenient on long-running projects:
...
```
$ z80map --bin rom.bin --symbols symbols.sym --out rom.asm
```
...
```
$ z80map --bin rom.bin --symbols symbols.sym --out rom.asm --fresh
```
...
```
; Auto-generated by z80map --symbolsout
...
`z80map.ts` (currently `writeCommentsOut(...)`) switches to the new
...
```
z80map: --cleanout-format maxam: this binary contains ZX Next opcodes
...
```
z80map: label 'ADD' at $BB15 is a reserved word in sjasmplus.
...
## 10. Manual line protection — user-overridable output blocks

### 10.1 Motivation

The disassembler's CFG analysis is deterministic: given the same binary
it always produces the same output. This is correct for code that is
reachable and decodable, but fails for:

- **Indirectly executed data** — bytes stored as a `DEFB` table at rest
  but copied to RAM and executed at runtime. The disassembler has no
  way to know they are code.
- **Encrypted regions** — e.g. Vortex-encoded operands where the
  decoder does not apply uniformly across all addresses.
- **Self-decrypting stubs** — code that patches itself before execution;
  static analysis sees the undecrypted bytes.
- Any other case where the user's manual knowledge of the binary
  exceeds what static analysis can determine.

In all these cases the user wants to **replace** a range of the auto-
generated listing with their own content, and have that content
**survive every subsequent re-run** unchanged.

### 10.2 Design decisions

| Concern | Decision |
|---------|----------|
| Scope of protection | Output-only. The analysis pass (CFG walk, CODE/DATA attribute marking, label creation) is completely unaffected. |
| Analysis labels at the block boundary | Auto-generated labels at the start address appear **above** the protect-start marker, produced normally by the analysis pass. |
| Verbose `.asm` output | Emit the stored manual lines verbatim, exactly as the user wrote them, in place of the auto-generated output for the address range. |
| `--cleanout` output | Extract instruction and directive lines from the manual content; strip address-prefix column and `;` comments; emit clean mnemonics. Comment-only lines are silently dropped. |
| Labels inside the block | **Opaque** — not registered in the disassembler's symbol table. Labels in the manual content are cosmetic only; they do not appear in call graphs or cross-reference lists. Users who need a label to be globally visible add it to their `--symbols` file instead. |
| Address range overlap / nesting | Disallowed. A protected block may not overlap another, or a registered data range. Report an error on import. |

### 10.3 Marker syntax

Two forms are accepted interchangeably. The user may use whichever they
prefer; they may also mix forms across different blocks.

#### Short form (recommended for everyday use)
...
- `;;{` followed by a space, then two plain 4-digit hex addresses
  separated by whitespace: `XXXX` = first protected address (inclusive),
  `YYYY` = last protected address (inclusive).
- `;;}` on a line by itself closes the block. No address is required.

Example:
```
;;{ C000 C005
C000 LD   A,#01         ;; init counter
C002 CALL $BB00
C005 RET
;;}
```
...
```
;;PROTECT-START XXXX YYYY
...manual content...
;;PROTECT-END
```

Identical semantics to the short form.

#### Validation

Both `;;{` and `;;}` are unambiguous in the existing grammar:

| Existing `;;` usage | Form | Conflict? |
|--------------------|------|-----------|
| Inline user note on an instruction | `MNEMONIC  ;; text` — always *after* an instruction, never at line start | ✗ |
| Orphan block lines | `;;ADDR content` — address hex followed by annotation | ✗ — `{`/`}` are not valid hex digits or keywords |
| Orphan block open/close | Dedicated lines starting with `;;` + specific keyword | ✗ — `{`/`}` are not those keywords |

The parser checks for `;;{` and `;;}` **before** all other `;;` handling,
so there is no ambiguity.

### 10.4 Verbose output behaviour

When `disassembleMemory()` walks the address range covered by a
protected block, it:

1. Emits the auto-generated label for the start address (if one exists)
   in the normal way — this appears **above** the protect-start marker.
2. Emits the `;;{` (or `;;PROTECT-START`) line.
3. Emits every stored manual content line verbatim.
4. Emits the `;;}` (or `;;PROTECT-END`) line.
5. Advances the walk pointer to `YYYY + 1` and continues normally.

Nothing inside the block is regenerated by the disassembler.

### 10.5 Clean output behaviour (`--cleanout`)

When the clean emitter reaches a protected address range it **does not**
emit raw `DEFB` bytes (as it would for an unprotected data region).
Instead it processes the stored manual content lines:

- Lines that are purely comments (`; ...` or `;;...` at line start) are
  **dropped**.
- Lines that carry an instruction or directive are emitted with:
  - The leading address prefix column stripped (e.g. `C000 ` removed).
  - Any inline comment (everything from `;` to end of line) stripped.
  - The remaining instruction/directive text emitted, tab-indented.
- Label lines (text ending with `:`) are emitted flush-left, as normal
  clean-output labels. Note: these labels are not in the disassembler's
  symbol table (§10.2) so they will not receive an EQU declaration if
  out-of-range — the user must ensure any label they write refers to
  an address within the loaded binary.

Example — manual content in the `.asm` file:

```
;;{ C000 C005
C000 LD   A,#01        ; 01h  ;; init counter
C002 CALL $BB00        ; BB00h
; this is a note
C005 RET
;;}
```

Resulting clean output:
...
### 10.6 Round-trip parser integration

The `asmClassifier` line classifier needs two new event kinds:

| Event | Line pattern | Payload |
|-------|-------------|---------|
| `protect-start` | `;;{ XXXX YYYY` or `;;PROTECT-START XXXX YYYY` | `{ startAddr: number, endAddr: number }` |
| `protect-end` | `;;}` or `;;PROTECT-END` | — |

Lines between a `protect-start` and the matching `protect-end` event
are classified as `protect-content` and stored verbatim (the full
original line, including address prefix and comments).

On import, the round-trip consumer builds a map:

```ts
protectedBlocks: Map<number, { endAddr: number; lines: string[] }>
```

keyed by `startAddr`.

### 10.7 Implementation sketch

**Phase 1 — Parser** (`asmClassifier.ts`):
- Add `protect-start` and `protect-end` event kinds.
- On `protect-start`: parse the two hex addresses; validate that YYYY ≥ XXXX.
- Between markers: emit each line as a `protect-content` event.

**Phase 2 — Importer** (`disasm.ts` round-trip consumer):
- Collect `protect-start`/`protect-content`/`protect-end` events into
  `this.protectedBlocks`.

**Phase 3 — Verbose emitter** (`disasm.ts` `disassembleMemory`):
- Before emitting any line at address `addr`, check
  `protectedBlocks.has(addr)`.
- If yes: emit the auto-generated label (if any) for `addr` as normal,
  then emit the stored lines verbatim, then advance past `endAddr`.

**Phase 4 — Clean emitter** (`cleanEmitter.ts`):
- Same check at the start of the data-region walk.
- If a protected block covers the current range, process the stored
  lines per §10.5 instead of the raw-byte grouping logic.

**Phase 5 — Tests**:
- Parser correctly recognises both short and long forms.
- Manual content survives a full round-trip (emit → re-parse → emit).
- Verbose output: auto-generated label above block, manual lines inside.
- Clean output: instructions extracted, comments dropped.
- Clean output: DEFB is NOT emitted for the protected range.
- Edge case: protected block at the very start of the binary (no label).
- Edge case: protected block immediately adjacent to another label.

### 10.8 Non-goals for this feature

- The disassembler does NOT validate that the manual content is valid
  Z80 assembly.
- The disassembler does NOT re-analyse the manual content to discover
  labels or cross-references.
- The disassembler does NOT enforce that the address range in the marker
  matches the actual addresses prefixed on lines inside the block. The
  address prefixes inside are cosmetic; the range is governed entirely
  by the marker.
