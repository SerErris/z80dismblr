## 3.4.0
- **New `--bytes raw|decoded|both` option** for the opcode byte column. `raw` (default, and the new behaviour of `--addbytes`) shows the bytes as physically stored in memory — the ROM image. `decoded` shows what the program effectively sees: M1 (opcode/prefix) bytes raw, operand and data bytes passed through the installed memory decoder (`--decoder`). `both` prints `raw | decoded` side by side, which makes the transform of an encrypted ROM (e.g. `--decoder vortex`) directly visible per instruction. The raw/decoded split mirrors exactly what the opcode decoder does, including the `DDCB`/`FDCB` case where the displacement and post-displacement selector are non-M1 reads. `--addbytes` is now a documented alias for `--bytes raw`. With no decoder installed, `decoded` is identical to `raw`. `both` roughly doubles the column width — raise `--clmnsbytes` if it overlaps the opcode field.

## 3.3.1
- **Fix: stale auto-generated local names were frozen as user renames on round-trip.** The round-trip importer only recognised a local auto-name whose parent prefix matched the *current* subroutine's name. Any local auto-named under a *different* parent number in a previous run — which happens whenever subroutine numbering shifts (subs added/removed/reordered, or parent re-attribution) — was misclassified as a deliberate user rename and locked (`isFixed`). E.g. an old `.asm` containing `.sub006_loop` kept emitting `.sub006_loop` even after the label was correctly re-attributed to `SUB004`. The detector now recognises the generic auto-local shape (`.sub<n>_l…`, `.lbl<n>_loop…`, `.rst<hh>_l…`, etc.) for any auto parent number and lets such names re-number freely; genuine user renames are still preserved.
- **Fix: local labels could be attributed to the wrong parent subroutine.** An indented local label inside one subroutine could be emitted with a different subroutine's prefix. Cause: `setSubroutineParent()` stopped its walk at *any* already-attributed address. When a subroutine's own branch recursion pre-set an address, the sequential walk hit it and bailed early, leaving a gap that a later, higher-numbered subroutine then claimed. The walk now only stops when it reaches code owned by a *different* subroutine; addresses already owned by the same subroutine are skipped over so the rest of the body stays correctly attributed.


## 3.3.0
- **External-memory labels (EXT / EXTSUB) now appear in `--symbolsout`.** Labels whose address falls outside the loaded binary — e.g. scratch-RAM variables (`LD (BE03),A`) or firmware entry points (`CALL B912`) — are now emitted to the `--symbolsout` skeleton file. They use a distinct `EXT` prefix (data) or `EXTSUB` prefix (code) to stay visually separate from in-ROM labels. Rename them in the skeleton, move to your `--symbols` file, and the name persists on every subsequent run. Dedup is preserved: once a label is in `--symbols` it is excluded from the next skeleton output.
- `isAutoGeneratedName()` updated to recognise `EXTnnn` / `EXTSUBnnn` so round-tripped auto-names are not mistakenly locked as user renames.
- User manual §10 and §14.3 updated; `--symbolsout` help text updated.


## 3.2.0
- **Local labels can be renamed and round-trip.** Renaming an indented local jump target in the `.asm` file (e.g. `.vdos_init_l1` → `.check_rom_zero`, keeping the leading dot) now locks the new name and reuses it on every subsequent run, including at the jump site. Local renames stay in the `.asm` file and are not written to `--symbolsout`. Unrenamed locals (names still matching the auto `.parent_l`/`.parent_loop` pattern) continue to re-number freely. A renamed local that collides with another label name produces a non-blocking stderr warning and is applied anyway.
- User manual §14.3 updated: corrected the auto-generated-name rules for local labels and documented the local-rename workflow.
