...
z80dismblr [options]
    ...
    Labels options:
        --symbols file: Input a symbol definition file.
            Maps addresses to names and provides structured metadata for
            subroutine headers (summary, entry/exit conditions, register
            contracts). Plain prose comments do not belong here — use the
            --out round-trip file for those.
            Format:
                "address [name]"          (hex address, optional label)
                "; summary: one-liner"    (structured sub-header field)
                "; action: description"
                "; entry:  DE = buffer"
                "; exit-success: Carry set"
                "; exit-failure: Carry clear"
                "; corrupted: A, BC, F"   (override analyser result)
                "; preserved: IX, IY"
                newline
            Example:
                BB00: KL_CHOKE_OFF
