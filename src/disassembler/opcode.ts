...
export class Opcode {
	...
	protected static convertToLabel(value: number): string {
		...
		if (!valueString) {
			valueString = Format.formatHex(value, 4);
		}
		return valueString;
	}
	...
	public disassemble(memory?: Memory): {mnemonic: string, comment: string} {
		...
		if (this.valueType == NumberType.CODE_LBL
			...
			valueName = Format.formatHex(this.value, 2);
		}
		else {
			// Use direct value
			const val = this.value;
			...
			if (this.valueType == NumberType.NUMBER_BYTE) {
				// byte
				valueName = Format.formatHex(val, 2);
				comment = Format.getVariousConversionsForByte(val);
			}
			else {
				// word
				valueName = Format.formatHex(val, 4);
		...
		if (!this.appendValueTypes) {
			...
			for (let k = 0; k < len; k++) {
				...
				let valName = (vType == NumberType.NUMBER_BYTE) ? Format.formatHex(val, 2) : Format.formatHex(val, 4);
				vals.push(valName);
			}
...
class OpcodeIndexImmediate extends Opcode {
	...
	public disassemble(memory?: Memory): {mnemonic: string, comment: string} {
		...
		const valueName = Format.formatHex(this.secondValue, 2);
...
class OpcodeNext_nextreg_n_a extends OpcodeNext {
	...
	protected static getRegisterName(regId: number): string {
		...
		switch (regId) {
			...
			default:
				// unknown
				regname = Format.formatHex(regId, 2);
				break;
		}
		return regname;
	}
...
class OpcodeNext_nextreg_n_n extends OpcodeNext_nextreg_n_a {
	...
	protected static getRegisterValueName(regId: number, regValue: number): string {
		...
		switch (regId) {
			...
			case 1: // REG_VERSION
				valuename = Format.formatHex(regValue, 2) + ' (v' + (regValue >>> 4) + '.' + (regValue & 0x0f) + ')';
				break;

			case 2: // REG_RESET
				valuename = Format.formatHex(regValue, 2);
